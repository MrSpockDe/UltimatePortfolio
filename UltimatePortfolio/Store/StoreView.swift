...
struct StoreView: View {
    ...
    var body: some View {
        ...
        .alert("In-app purchases are disabled.", isPresented: $showingPurchaseError) {
            ...
            Text("""
            You can't purchase the premium unlock, because ia-app purchases are disabled on this device.

            Please ask whomever manages your device for assistance.
            """)
        }
        .onChange(of: dataController.fullVersionUnlocked) { checkForPurchases() }
        .task {
            await load()
        }
    }
